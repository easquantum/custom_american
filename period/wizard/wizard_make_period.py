...
period_fields = {
    'part':{
        'string':"Periods",
        'type':'selection',
        'selection':[('1','Monthly'),('3','3 Monthly')],
        'default': lambda *a:'1'
    },
    ...
    'type':{
        'string':"Type",
        'type':'selection',
        'selection':[('stock','Pickings'), ('purchase','Purchase Orders'), ('sale','Sale Orders'), ('all','All')],
...
def _do_period(self, cr, uid, data, context):
    pgy_obj = pooler.get_pool(cr.dbname).get('period.generalyear')
    pgy = pgy_obj.browse(cr, uid, [data['id']])[0]

    interv = int(data['form']['part'])
    tp = data['form']['type']
    pgp = pgy.create_period(context,interv,tp)

    return {}

class wiz_period_by_year(wizard.interface):
	states = {
		'init': {
			'actions': [],
...
wiz_period_by_year("period.wizard.period_year")
