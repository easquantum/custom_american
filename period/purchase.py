...
class purchase_order(osv.osv):
    _description = "purchase with general period"
    ...
    def _check_date(self,cr,uid,ids):
        ...
        for purchase in self.browse(cr,uid,ids):
            ...
            if date:
                crit = [('date_start','<=',date),('date_stop','>=',date),('state','=','draft'),('type','=','purchase')]
    ...
    _constraints = [
        (_check_date, 'Purchase Order Error ! Could not validate the date, The date is not in an open period, please change the date ', ['date'])
    ]
