...
data_form = '''<?xml version="1.0"?>
	<form string="Calculo de Comisiones Zona">
		<field name="zone_id"/>
...
def _commissions_calculated_seller(cr, uid,  zona, desde,hasta, nomb, dias,period_id,context={}):
    ...
    if zona:
        ...
        vals['cash_pay']  = 0
        vals['pay_total'] = 0
        if parameters_zone.amount_cash > 0:
            vals['cash_percent'] = vals['cash_total'] /  parameters_zone.amount_cash * 100
            if vals['cash_percent'] > parameters_zone.percent_max:
                vals['cash_percent'] = parameters_zone.percent_max
            vals['cash_pay'] = vals['cash_percent'] * parameters_zone.value_cash /100
        if parameters_zone.amount_total > 0:
            vals['sale_percent'] = vals['sale_total'] / parameters_zone.amount_total * 100
...
class commissions_calculated_zone(wizard.interface):
...
commissions_calculated_zone("commissions_seller_zone")
