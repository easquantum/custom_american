...
data_fields = {
	'zone_id':   {'string':'Division', 'type':'many2one', 'relation':'res.partner.zone', 'required':True, 'domain':[('type','=','division')]},
...
#
#
def _commissions_calculated(cr, uid, div_id, zone_ids, period_id, nomb, dias,cuota_percent):
    #Cuota
    cuota_year    = 0
    cuota_month   = 0
    ...
    if parameters_div and parameters_div.seller_id.id:
        ...
        cuota_year    =parameters_div.amount_total
    ...
    if datosped:
        totaldesp = datosped[0][0]
        if cuota_month > 0:
            sale_percent = totaldesp / cuota_month * 100
            if sale_percent < parameters_div.percent_min:
                sale_percent = 0
...
def _commissions_calculated_seller_div(self, cr, uid, data, context={}):
    ...
    for d in division_ids:
        ...
        datos  = _commissions_calculated(cr, uid, d, zone_ids, period_id, nomb, dias,cuota_percent)
    return {}
 

class commissions_invoice_calculated_division(wizard.interface):
...
commissions_invoice_calculated_division("commissions_invoice_division")
