...
data_fields = {
	'ter_id':   {'string':'Territorio', 'type':'many2one', 'relation':'res.partner.zone','required':True,'domain':[('type','=','territory')]},
	'number_day': {'string':'Nro. Domingos', 'type':'integer', 'required':True},
	'amount': {'string':'cuota', 'type':'float', 'required':True},
	'commissions_type': {'string':'Comision Tipo:', 'type':'selection', 'readonly':1, 'default': lambda *a: 'territory','selection':[('territory','Territorio')]},
...
#
#
def _collection_calculated_territory(cr, uid, ter_id, zone_ids, period_id, nomb, dias,tipo,cuota,partner_id,desde,hasta):
...
def _commissions_collection_calculated_ter(self, cr, uid, data, context={}):
    ...
    if partner_id and partner_id[0]: 
        ...
        return {'resultado':"No hay Gerente Territorial Asignado, por favor signar uno y procesar nuevamente...!"}


    zone_ids = []
    if ter_id:
        zone_ids = pooler.get_pool(cr.dbname).get('res.partner.zone').search(cr, uid, [('parent_id','=',ter_id)])
        datos  = _collection_calculated_territory(cr, uid,ter_id,zone_ids,period_id,nomb,dias,tipo,cuota,partn_id,desde,hasta )
    return {}
 

class commiss_collection_calc_territory(wizard.interface):
	...
	states = {
        ...
        'finish':{
            'actions':[_commissions_collection_calculated_ter],
            'result':{'type':'form', 'arch':_result_form, 'fields':_result_fields, 'state':[('end','OK')]}
        },		
	}

commiss_collection_calc_territory("commissions_collection_calc_ter")
