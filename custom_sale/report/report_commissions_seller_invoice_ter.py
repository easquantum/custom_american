...
class comisiones_invoice_ter(report_sxw.rml_parse):
    #Variables Globales----------------------------------------------------
    currentId	= 0
    total	    = 0
    amount_total= 0
    ...
    def __init__(self, cr, uid, name, context):
		super(comisiones_invoice_ter, self).__init__(cr, uid, name, context)
    ...
    def _set_parameters(self,zone_id):
        ...
        parameters	 = pooler.get_pool(self.cr.dbname).get('parameters.seller.zone').read(self.cr, self.uid,  parameter_id,['amount_total','value_total'])
...
report_sxw.report_sxw('report.commissions_invoice_ter','commissions.seller','addons/custom_american/custom_sale/report/report_commissions_seller_invoice_ter.rml',parser=comisiones_invoice_ter, header=False)
