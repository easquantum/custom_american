...
class pedido_venta(report_sxw.rml_parse):
	...
	def _get_detalle(self, order):		
		...
		if not resultSQL:
			product_fact	= [{"cod":'',"nomb":'',"ref":'',"cant":0,"precio":0,"total":0,"alic":0,"imp":''}]
		...
		for inf in resultSQL:
			...
			for tx in self.cr.fetchall():
			    if tx:
			        alic  = 100 * tx[0]
			tax_line 	= pooler.get_pool(self.cr.dbname).get('sale.order.line').read(self.cr, self.uid, [inf[5]],['tax_id'])
			...
			if (inf[3] > 0) and (inf[4] > 0):
				...
				self.subtotal += total						
			product_fact.append({"cod":inf[0],"nomb":inf[1],"ref":inf[2],"cant":inf[4],"precio":inf[3],"total":total,"alic":alic,"imp":impt})	
		return product_fact		
