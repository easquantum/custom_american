...
def _refund_print_internal(self, cr, uid, data, context):
	...
	if datosg:
		...
		mes			= _set_format_string('',3,'center')		#Mes  
		...
		totalpagar	= ''.rjust(11)						#Total Documento
		monto_fact  = _set_format_string('0.00',20,'ljust')			#Total Factura
		...
		if idfa:  		
		    fa_dat		= pooler.get_pool(cr.dbname).get('account.invoice').read(cr, uid, [idfa],['date_invoice','amount_total'])
		    if fa_dat and fa_dat[0]:
		        ffa     = fa_dat[0]['date_invoice']
		        fechafa = ffa.split('-')
		        ffa		= fechafa[2] +'-' + fechafa[1] +'-' + fechafa[0]
		        ffa  = ffa.ljust(20)
		        monto_fact = fa_dat[0]['amount_total']
	...
	output.write(dir_fis2+dir_ent3+'Fecha Fact.:     Factura:          Bs Factura:\n')
	output.write(dir_fis3+cdad_ent+ffa+nrop+monto_fact+'\n')
	output.write(loc_fis+edo_ent+'Fecha N/C:'+fechaf+'\n')
	...
	for t in tax_line:
		...
		nb_iva = t[2].rstrip()
		output.write(comprime+'                                                                                      '+normal+'       IVA  12%           '+mnt_iva+'\n')
