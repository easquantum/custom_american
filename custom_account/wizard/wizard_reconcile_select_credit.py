...
_journal_fields = {
    'account_id': {'string':'Account', 'type':'many2one', 'relation':'account.account','domain': [('reconcile','=',1),('parent_id','child_of',[1179,2297])], 'required':True},
}
...
class wiz_rec_select_credit(wizard.interface):
    states = {
        'init': {
            'actions': [],
...
wiz_rec_select_credit('reconcile.select.credit')
