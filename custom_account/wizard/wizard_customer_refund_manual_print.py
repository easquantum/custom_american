...
<form string="Imprimir Notas Credito Interna Manual">
...
TheFields = {
	...
	'invoice': {'string':'Nota Credito', 'type':'many2one','relation': 'account.invoice', 'required':True,  'size':90,  'domain':[('type','=','out_refund'),('internal','=',1),('manual','=',1)] },
}
...
def _refund_print_manual(self, cr, uid, data, context):
	#####Variables
	...
	if datosg:
		...
		mes			= _set_format_string('',4,'center')					#Mes  
		nrop		= _set_format_string(datosg[5],23,'ljust')			#Nro. Factura Afectada
		f		    = datosg[6].split('-')
		fechaf		= f[2] +'-' + f[1] +'-' + f[0]
		...
		if reslt:
			...
			edo_ent		= _set_format_string(datos_e[3],35,'ljust')
	...
	#Consulta de datos Detalle de la Nota Credito
	sqlp = """
	...
	if result_d:
		for inf in result_d:
			...
			descrip	= inf[1] 						
			descrip	= descrip.ljust(96)								#Referencia
			cantidad	= locale.format('%.0f',  inf[2], grouping=True)	#Cantidad
			cantidad	= cantidad.replace(',','.')					#
			cantidad	= cantidad.center(5)						#
	...
	output.write(dir_fis2+dir_ent3+'Factura Nro.:          Fecha N/C:\n')
	output.write(dir_fis3+cdad_ent+nrop+fechaf+'\n')
	output.write(loc_fis+edo_ent+ntline+'\n')
	...
	#Datos del Detalle
	output.write(normal+"-------------------------------------------------------------------------------\n")	
	encab = 'DESCRIPCION                                     CANT.     PRECIO     IMPORTE  %'
	...
	#Totales 	
	output.write(comprime+'                                                                                      '  + normal + '      TOTAL N.C        Bs.'+ subtotal + '\n')
	output.write(comprime+'                                                                                    ' + normal + '        Menos Dcto.    '+vdscto+' '+ totaldscto+ '\n')
	output.write(comprime+'                                                                                    '+ normal + '            Subtotal    Bs.'+ factotal + '\n')
	...
	for t in tax_line:
		...
		nb_iva = t[2].rstrip()
		output.write(comprime+'                                                                                      '+normal+'            IVA '+iva+' %  '+mnt_iva+'\n')
...
class manual_refund_prints(wizard.interface):
	...
	states = {
		...
		'report' :  {
			'actions' : [_refund_print_manual],
			'result': {'type':'form', 'arch':_result_form, 'fields':_result_fields, 'state':[('end','OK')]}
		},		
	}

manual_refund_prints("manual_invoice_refund_print")