...
class libro_ventas(report_sxw.rml_parse):
    ...
    def _set_datos_tax_nc(self,alicnc,ivanc,basenc):
        ...
        if not self.datosnctax:
            self.datosnctax	= [{'tax':alicnc,'totaltax':ivanc,'totalbase':basenc,'totalret':ivaretnc}]
        else:
    ...
    def _get_libro_ventas(self, form):
        ...
        sql = """
        SELECT a.id,a.date_invoice,a.name,a.type,a.state,a.amount_total,a.amount_tax,p.id,p.vat,a.parent_id     
