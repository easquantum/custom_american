...
class ventas_netas_zona(report_sxw.rml_parse):
	#Variables Globales----------------------------------------------------
	totalcajas	= 0
	totalventas	= 0
	totaldscto	= 0
	...
	def __init__(self, cr, uid, name, context):
		super(ventas_netas_zona, self).__init__(cr, uid, name, context) 
	...
	def _get_detalle(self, form):		
		...
		sql = """
		SELECT d.product_id,r.id,r.name,p.default_code,t.name,p.variants,sum(d.quantity),t.list_price,i.amount,z.cod  
		FROM		account_invoice				AS a
		INNER JOIN	res_partner_zone			AS z ON a.code_znoe_id=z.id
		...
		GROUP BY z.cod,d.product_id,r.id,r.name,p.default_code,t.name,p.variants,t.list_price,i.amount  
		ORDER BY z.cod,r.name,p.default_code;"""%(fdesde,fhasta)
		#print sql
		self.cr.execute(sql)
	...
	def _get_zone(self,zone):
		nombre = '' 
		if zone:
			user	= pooler.get_pool(self.cr.dbname).get('res.users').read(self.cr, self.uid, [zone],['name'])								
			nombre	= user[0]['name']
		return nombre			
	
report_sxw.report_sxw('report.ventas_netas_zona','account.invoice','addons/custom_american/custom_account/report/ventas_netas_zona.rml',parser=ventas_netas_zona, header=False)
