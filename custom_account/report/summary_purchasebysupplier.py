...
class summary_purchase_supplier(report_sxw.rml_parse):
	...
	def __init__(self, cr, uid, name, context):
		super(summary_purchase_supplier, self).__init__(cr, uid, name, context)
	...
	def _get_purchase(self,form):
		...
		for invoice in resultSQL:
			#Inicializacion Variables de Totales---------------------------------------------------------------------------------------------
			total		= 0
			iva			= 0
			totalpp		= 0
			totalcta	= 0
			...
			if result_dscto: 
				...
				if ndeb=='S':
					totalcta = totalcompra * result_dscto[0][3] * -1   #Si tiene nota de debito, se recalcula total de la cta
				totalreserva = 0
				ctarsv='S'
...
report_sxw.report_sxw('report.summary_purchasebysupplier','account.invoice','addons/custom_american/custom_account/report/summary_purchasebysupplier.rml',parser=summary_purchase_supplier, header=False)
