...
class factura_venta(report_sxw.rml_parse):
	address_delivery = []
	currentId = 0
	totaliva = 0
	iva		= 0
	...
	def __init__(self, cr, uid, name, context):
		super(factura_venta, self).__init__(cr, uid, name, context)
	...
	def _get_notas_atencion(self,invoice):
		ntline	= ''
		observ = pooler.get_pool(self.cr.dbname).get('account.invoice').read(self.cr, self.uid, [invoice],['comment'])
		if observ[0]['comment']:
			ntline	= observ[0]['comment']  
		sql = """	
		SELECT n.code,n.name 
		FROM	 account_nota_atencion_rel	AS r
		INNER  JOIN nota_atencion			AS n	 ON r.nota_atencion_id=n.id
		WHERE r.invoice_id=%d ;"""%invoice
		self.cr.execute (sql)
		for nt in self.cr.fetchall():
			ntline	+= '  N-A='+nt[0]	
		return ntline
	...
	def _get_totaliva(self):
		self.totaliva = (self.subtotal - self.totaldcto) * self.iva / 100
		return self.totaliva
		
	def _get_total(self):
		self.totalgral = self.subtotal - self.totaldcto + self.totaliva
		return self.totalgral

	def _get_totaldcto(self):
		return self.totaldcto		
		
report_sxw.report_sxw('report.factura_ventas','account.invoice','addons/custom_american/custom_account/report/report_factura_ventas.rml',parser=factura_venta, header=False)
