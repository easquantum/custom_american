...
class libro_compras(report_sxw.rml_parse):
    ...
    def _procesar_datos(self,listado):	
        ...
        for ids in listado:
            ...
            fecha       = invoices.date_document                     #Fecha del Documento [FACT - NC]
            nrocomp     = invoices.number_retention                  #Nro Comprobante Retencion
            nrofac		= invoices.number_document                   #Nro de FACT
            nronc		= ''                                         #Nro Nota Credito
            ...
            #Razon Social: La razon social debe estar definida en los contactos con el tipo default
            sqlrzs="SELECT name FROM res_partner_address WHERE type='default' AND partner_id=%d"%partner_id 
            self.cr.execute (sqlrzs)
            rzsocial_info = self.cr.fetchall()
            if rzsocial_info and rzsocial_info[0] and rzsocial_info[0][0]:
                razonsocial	= rzsocial_info[0][0]
            #print "ID",invoice_id,"  NroF",nrofac," FEC",fecha,"  COMPRET",nrocomp
            if invoices.amount_tax:
                ...
                for tax in result_tax:
                    ...
                    if tax_info[0]['tax_group'] == 'vat':
                        ...
                        cont += 1
                        libroCompras.append({"cont":cont,"fecha":fecha, "rif": rifp, "proveedor":razonsocial, 'tipo':tipodoc, 'nrocomp':nrocomp,'nrofac':nrofac, "nronc":nronc, "nrocont":nrocont ,"afect":nrofa, 'total':total, 'exento':exento, 'base':base, 'alicuota':alicuota, 'iva':iva, 'ivaret':ivaret})
            ...
            if exentas:
                ...
                if invoices.type == 'in_refund':
                    ...
                    if invoices.parent_id.id and invoices.parent_id.number_document:
                        nrofa    = invoices.parent_id.number_document                  #Nro Factura Afectada
                    self.ttexento	-= exento
                    self.ttcompra	-= total
                    self.ttncexento	+= exento
                else:
    ...
    def _get_libro_compras(self, form):
        ...
        ORDER BY a.date_document,a.name;"""%(filtro,fdesde,fhasta)	
        #print sql 
        self.cr.execute (sql)
        ...
        if listado:
            ...
            return [{'cont':'','fecha':'','rif':'','proveedor':'','tipo':'','nrocomp':'','nrofac':'','nronc':'','nrocont':'','afect':'','total':0,'exento':0,'base':0,'alicuota':0,'iva':0,'ivaret':0}]
        #Creado archivo 
...
report_sxw.report_sxw('report.libro_compra','account.invoice','addons/custom_american/custom_account/report/libro_compras.rml',parser=libro_compras, header=False)
