...
<form string="Declaracion ISLR SENIAT">
	...
	<field name="date1"/>
	<field name="date2"/>
	<separator colspan="4" string="Proveedor:"/>
...
data_fields = {
	...
	'date2': {'string':'Hasta', 'type':'date', 'required':True, 'default': lambda *a: time.strftime('%Y-12-31')},
	'supplierid': {'string':'Proveedor', 'type':'many2one','relation': 'res.partner', 'required':True, 'domain':[('supplier','=',1)], 'size':132 }
}


class islr_arcv(wizard.interface):
	...
	states = {
		...
		'report' : {
			'actions' : [],
			'result': {'type': 'print', 'report': 'islr_arcv', 'state': 'end'}
		},
	}
